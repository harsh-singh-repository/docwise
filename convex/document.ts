...
export const getSideBar =  query({
    ...
    handler:async(ctx,args)=>{
      ...
      const document = await ctx.db.query('documents').withIndex('by_user_parent',(q) =>
      q
       .eq('userId',userId)
...
export const getTrash = query({
    handler:async(ctx) => {
     ...
     const document = await ctx.db.query("documents").withIndex("by_user",(q) => q.eq("userId",userId)).filter((q) => q.eq(q.field("isArchived"),true)).order("desc").collect();
